...
if __name__ == '__main__':
    ...
    parser.add_argument('--rollback',
                        type=str,
                        choices=['none', 'stochastic', 'deterministic'],
                        default = 'none',
                        help = "Rollback method for advanced experiment")
    ...
    act = args.act
    fusion = args.fusion
    rollback = 'rollback-'+args.rollback
    beta = args.beta
    print(args)
    
    
    # load torch dataset 
    data_path = f'./data/torch_data_{starting_date}_{rollback}.pickle'
    print(data_path)
    ...
    model_path = f'./saved_models/DATE_{starting_date}_{rollback}_{exp_id}.pkl'
    ...
    train(args)
    overall_f1, auc, precisions, recalls, f1s, revenues = evaluate(save_model, exp_id)
    ...
    model_name = f'DATE_{starting_date}_{rollback}'
    
    if os.path.isfile(output_file):
        print("Adding result on file")
       
        with open(output_file, 'a') as ff:
            
             # Write on file
            print(f'{starting_date},{overall_f1},{auc},{precisions[0]},{precisions[1]},{precisions[2]},{precisions[3]},{recalls[0]},{recalls[1]},{recalls[2]},{recalls[3]},{revenues[0]},{revenues[1]},{revenues[2]},{revenues[3]},{model_name},{epochs},{dim},{lr},{weight_decay},{beta},{head_num},{fusion},{act}', file=ff)
            
             # Print on screen
            print("""Model:%s Starting_date:%s\n Metrics:\nf1:%.4f auc:%.4f\nPr@1:%.4f Pr@2:%.4f Pr@5:%.4f Pr@10:%.4f\nRe@1:%.4f Re@2:%.4f Re@5:%.4f Re@10:%.4f\nRev@1:%.4f Rev@2:%.4f Rev@5:%.4f Rev@10:%.4f""" \
              % (model_name, starting_date, overall_f1, auc,\
        ...
        print("File not exist, Make a file and add result on file")
        with open(output_file, 'a') as ff:
            
            # Write on file
            print("starting_date,f1,auc,pr1,pr2,pr5,pr10,re1,re2,re5,re10,rev1,rev2,rev5,rev10,model,epoch,dim,lr,l2,beta,heads,fusion,activation", file=ff)
            print(f'{starting_date},{overall_f1},{auc},{precisions[0]},{precisions[1]},{precisions[2]},{precisions[3]},{recalls[0]},{recalls[1]},{recalls[2]},{recalls[3]},{revenues[0]},{revenues[1]},{revenues[2]},{revenues[3]},{model_name},{epochs},{dim},{lr},{weight_decay},{beta},{head_num},{fusion},{act}', file=ff)
...
#     with open(output_file, 'a') as ff:
#         print()
#         print("""Starting_date:%s\n Metrics:\nf1:%.4f auc:%.4f\nPr@1:%.4f Pr@2:%.4f Pr@5:%.4f Pr@10:%.4f\nRe@1:%.4f Re@2:%.4f Re@5:%.4f Re@10:%.4f\nRev@1:%.4f Rev@2:%.4f Rev@5:%.4f Rev@10:%.4f""" \
#               % (starting_date, overall_f1, auc,\
#                  precisions[0],precisions[1],precisions[2],precisions[3],\
#                  recalls[0],recalls[1],recalls[2],recalls[3],\
#                  revenues[0],revenues[1],revenues[2],revenues[3]
#                  ),
#                  ) 
#         model_name = f'DATE_{starting_date}'
