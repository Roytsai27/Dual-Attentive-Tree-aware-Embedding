...
class Attention(nn.Module):
    ...
    def forward(self, query,key): # assume key==value
        batch,time_step,dim = key.size(0) ,key.size(1), key.size(2)
        
        # concate input query and key 
        query = query.view(-1,1,dim)
        query = query.expand(batch,time_step,dim)
...
class AttentionalTreeEmbeddig(nn.Module):
    def __init__(self,max_leaf,importer_size,item_size,dim,head_num=4,fusion_type="concat",act="relu",device="cpu"):
    ...
    def eval_on_batch(self,test_loader): # predict test data using batch 
        ...
        for batch in test_loader:
            ...
            y_pred2 = y_pred_rev.detach().cpu().numpy().tolist()

            final_output.extend(y_pred)
            final_output2.extend(y_pred2)

        print("CLS loss: %.4f, REG loss: %.4f"% (np.mean(cls_loss), np.mean(reg_loss)))
        return np.array(final_output).ravel(), np.array(final_output2).ravel(), np.mean(cls_loss)+ np.mean(reg_loss)
