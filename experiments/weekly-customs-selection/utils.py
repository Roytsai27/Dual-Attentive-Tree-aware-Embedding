...
def torch_threshold(y_pred_prob,y_test,best_thresh = None):
    threshold_list = np.arange(0.1,1.01,0.1)
    best_f1 = 0
    if best_thresh == None:
        for th in threshold_list:
            y_pred_label = (y_pred_prob > th)*1 
...
def fgsm_attack(model, loss, images, labels, eps) :
    ...
    outputs = model.pred_from_hidden(images)
...
def writeResult(test_pred, method, input_path, output_path):
    if os.path.exists(output_path):
        df = pd.read_csv(output_path)
    else:
        df = pd.read_csv(input_path)
        df = df[['SGD_NUM','SGD_DATE','IMPORTER_TIN','DECLARANT_CODE','OFFICE_CODE','ORIGIN_CODE','HSCODE','ITM_NUM']]
    assert len(df) == len(test_pred)
    rank = rankdata(-1*test_pred, method='min')
    assert np.argmax(test_pred) == np.argmin(rank)
    
    df[method] = test_pred
    df['Ranking-'+method] = rank
    
    df.to_csv(output_path, index=False)
#     df.to_csv(output_path, index=False, float_format='%.f')
